...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((acc, value) => acc + value.charCodeAt(0), 0);
}
...
function extractNameFromTemplate(value) {
  ...
  return null; // Return null if the template string doesn't match the expected pattern
}
...
function getCardId(value) {
  ...
  if (!ranks.includes(value.slice(0, -1)) || !suits.includes(value.slice(-1))) {
    return 'Invalid Card';
  }

  const rankIndex = ranks.indexOf(value.slice(0, -1));
  const suitIndex = suits.indexOf(value.slice(-1));
  const cardId = rankIndex + suitIndex * ranks.length + 1;
